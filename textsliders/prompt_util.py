...
class PromptSettings(BaseModel):  # yaml
    ...
    reference_images: Optional[Dict] = None  # 参考图像配置
    mrsa_config: Optional[Dict] = None       # MRSA配置
...
class PromptEmbedsPair:
    ...
    #! 需要修改的是target_latents，加上参考图的条件
    #! 不确定需不需要修改positive_latents，positive_latents需不需要也使用参考图
    #! unconditional_latents和neutral_latents不使用参考图，使用代理概念
    def _erase(
        self,
